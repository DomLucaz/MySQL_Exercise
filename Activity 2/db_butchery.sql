...
CREATE TABLE tb_produtos (
	...
	id bigint auto_increment,     /* o auto_increment contará de forma sequencial e automatica o id */
...
SELECT nome "Produto", concat('R$', preco) "Preço", qtproduto "Quantidade no Estoque", DATE_FORMAT(dtvalidade, "%d/%m/%Y") "Validade" FROM tb_produtos
WHERE nome LIKE 'C%';  /* Para buscar um valor (nome) que tem uma inicial com C, mas se tiver o % no inicio e no fim %  a busca vai ser a busca do valor em qualquer localização, caso tiver % no final é para buscar o valor que estiver no fim */
